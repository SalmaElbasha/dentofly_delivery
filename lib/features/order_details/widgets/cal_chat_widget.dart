...
class CallAndChatWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    String? phone = isSeller? orderModel!.sellerInfo!.phone :(orderModel?.isGuest??true) ? orderModel?.shippingAddress?.phone: orderModel!.customer?.phone??'';
    int? id = 0;
    String? name = '';
    if(isAdmin){
      id = 0;
    ...
    return Row(children: [
      ...
      if(isAdmin || isSeller || !(orderModel?.isGuest??false))
      InkWell(
        onTap: (){
        if(!isSeller && !isAdmin && (orderModel?.isGuest??false)){
          showCustomSnackBarWidget('you_cant_chat_with_guest_user'.tr);
        }
          else if(!isSeller && !isAdmin && !(orderModel?.isGuest??false)){
